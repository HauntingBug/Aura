...
app.put("/api/codes/create", verifyApikey, async (req, res) => {
    ...
    if (!code || !ip || !port || !accountId)
        return res.status(400).json({ error: "Missing parameter: " + (!code ? "code" : !ip ? "ip" : !port ? "port" : !accountId ? "accountId" : "") });
    const user = await Users.findOne({ accountId: accountId });
    if (!user)
        return res.status(404).json({ error: "User not found" });
    if (code.length > 16)
        return res.status(400).json({ error: "Your code can't be longer than 16 characters" });
    if (code.length < 4)
        return res.status(400).json({ error: "Your code has to be at least 4 characters long." });
    if (code.includes(" "))
        return res.status(400).json({ error: "Your code can't contain spaces" });
    if (/[~`!#$%\^&*+=\-\[\]\\';,/{}|\\":<>\?]/g.test(code))
        return res.status(400).json({ error: "Your code can't contain any special characters" });
    const ipExp = new RegExp("^((25[0-5]|(2[0-4]|1[0-9]|[1-9]|)[0-9])(\.(?!$)|$)){4}$");
    if (!ipExp.test(ip))
        return res.status(400).json({ error: "Your provided an invalid ip adress" });
...
app.delete("/api/codes/delete", verifyApikey, async (req, res) => {
    ...
    if (!code || !accountId)
        return res.status(400).json({ error: "Missing parameter: " + (!code ? "code" : !accountId ? "accountId" : "") });
    const user = await Users.findOne({ accountId: accountId });
    if (!user)
        return res.status(404).json({ error: "User not found" });
    ...
    // @ts-expect-error
    if (codeExists.owner?.accountId !== user.accountId)
        return res.status(403).json({ error: "You don't own this code as the owner is " + codeExists.owner });
