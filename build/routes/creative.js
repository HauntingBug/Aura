...
app.get("/links/api/fn/mnemonic/:islandCode", async (req, res) => {
    const islandCode = req.params.islandCode;
    ...
    const islandData = await Island.findOne({ menmonic: islandCode }).then((island) => {
        if (!island) {
            return error.createError("errors.com.epicgames.links.no_active_version", "No active links available for mnemonic: " + islandCode, [], 1, "no_active_link", 404, res);
        }
        res.json(island);
    }).catch((err) => {
    });
