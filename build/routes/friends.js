...
app.all("/friends/api/v1/*/friends/:friendId/alias", verifyToken, getRawBody, async (req, res) => {
    ...
    if (!friends?.list.accepted.find(i => i.accountId == req.params.friendId))
        return error.createError("errors.com.epicgames.friends.friendship_not_found", `Friendship between ${req.user.accountId} and ${req.params.friendId} does not exist`, [req.user.accountId, req.params.friendId], 14004, undefined, 404, res);
    ...
    switch (req.method) {
        case "PUT":
            if (!(aliasPattern.test(req.rawBody)) || (req.rawBody.length < 3) || (req.rawBody.length > 16))
                return error.createError("errors.com.epicgames.validation.validation_failed", "Validation Failed. Invalid fields were [alias]", ["[alias]"], 1040, undefined, 404, res);
