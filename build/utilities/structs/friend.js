...
class friendsClass {
    async validateFriendAdd(accountId, friendId) {
        let sender = await Friends.findOne({ accountId: accountId }).lean();
        let receiver = await Friends.findOne({ accountId: friendId }).lean();
        if (!sender || !receiver)
            return false;
        if (sender.list.accepted.find(i => i.accountId == receiver?.accountId) || receiver.list.accepted.find(i => i.accountId == sender?.accountId))
            return false;
        if (sender.list.blocked.find(i => i.accountId == receiver?.accountId) || receiver.list.blocked.find(i => i.accountId == sender?.accountId))
            return false;
        if (sender.accountId == receiver.accountId)
            return false;
        return true;
    }
    async validateFriendDelete(accountId, friendId) {
        let sender = await Friends.findOne({ accountId: accountId }).lean();
        let receiver = await Friends.findOne({ accountId: friendId }).lean();
        if (!sender || !receiver)
            return false;
    ...
    async validateFriendBlock(accountId, friendId) {
        let sender = await Friends.findOne({ accountId: accountId }).lean();
        let receiver = await Friends.findOne({ accountId: friendId }).lean();
        if (!sender || !receiver)
            return false;
    ...
    async sendFriendReq(fromId, toId) {
        if (!(await this.validateFriendAdd(fromId, toId)))
            return false;
    ...
    async acceptFriendReq(fromId, toId) {
        if (!(await this.validateFriendAdd(fromId, toId)))
            return false;
        let from = await Friends.findOne({ accountId: fromId });
        let fromFriends = from?.list;
    ...
    async deleteFriend(fromId, toId) {
        if (!(await this.validateFriendDelete(fromId, toId)))
            return false;
        let from = await Friends.findOne({ accountId: fromId });
        let fromFriends = from?.list;
        let to = await Friends.findOne({ accountId: toId });
        ...
        for (let listType in fromFriends) {
            ...
            if (listType == "blocked")
                continue;
            if (findToFriend != -1)
                toFriends[listType].splice(findToFriend, 1);
        }
        if (removed == true) {
            functions.sendXmppMessageToId({
                "payload": {
    ...
    async blockFriend(fromId, toId) {
        if (!(await this.validateFriendDelete(fromId, toId)))
            return false;
        if (!(await this.validateFriendBlock(fromId, toId)))
            return false;
